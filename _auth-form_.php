 ...
 <body  class="login-page" onload="masquer();">
    <div style="box-shadow: 1px 1px 13px #555;" class="login-box">
      ...
      <div class="login-box-body">
        <p style="color:red;" class="login-box-msg"><b>ESPACE FORMATEUR</b></p>
        ...
        {           
          ...
          //test de connexion pour l'agent commercial
          $resultats = $bd->bdd->query("SELECT * FROM useradmin WHERE loginCpte='" . $login . "' && pwdCpte='" . $pass . "' && fctCpte='Enseignant' ") or die(mysql_error());
          $req = $resultats->fetch();
          ...
          else
          {
            if( sha1($req["pwdCpte"]) == sha1($pass) && sha1($req["loginCpte"]) == sha1($login) )
            {
              ...
              header("location:_start-form_.php");    
            }
            else
            {
      ...
      ?>
        ...
        <form action=" " method="post">
          <div class="form-group has-feedback">
            <input type="text" name="login" class="form-control" placeholder="Entrer votre nom d'utilisateur" onclick="erreur();" required />
